...
namespace Business.Concrete
{
    public class WareHouseService : IWareHouseService
    {
        ...
        public async Task<ApiResponse<NoData>> AddWareHouse(PostWareHouse postWareHouse, long currentUserId)
        {
        ...
        public async Task<ApiResponse<NoData>> DeleteWareHouse(int id, long currentUserId)
        {
            var getUser = await _userRepository.GetByIdAsync(currentUserId);
            if (getUser == null)
            {
        ...
        public async Task<ApiResponse<List<GetWareHouse>>> GetListWareHouseAsync(long currentUserId)
        {
            var getUser = await _userRepository.GetByIdAsync(currentUserId);
            if (getUser == null)
            {
            ...
            var getList =  await _wareHouseRepository.GetAllAsync(p=>p.IsActive==true);
        ...
        public async Task<ApiResponse<NoData>> UpdateWareHouseAsync(UpdateWareHouse updateWareHouse, long currentUserId)
        {
            var getUser = await _userRepository.GetByIdAsync(currentUserId);
            if (getUser == null)
            {
